...
export function processCryptoError(err) {
    throw new DataError(DataErrors.CRYPTOGRAPHIC_ERROR, err);
}

export function processMongoError(err) {
...
export function processCacheError(err) {
    throw new DataError(DataErrors.CACHE_ERROR, err);
}

export function processTokenError(err) {
    switch (err.message) {
        case "jwt expired":
...
export function processAWSError(err) {
    throw new DataError(DataErrors.AWS_S3_ERROR, err);
}

export function processImageError(err) {
    throw new DataError(DataErrors.IMAGE_PROCESSING_ERROR, err);
}

export function processValidationError(err) {
    if (err.isJoi) {
